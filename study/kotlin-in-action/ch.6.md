...
`Int?`, `Boolean?` 같은 널 타입은 코틀에서는 어떻게 컴파일할까?
...
코드에서 동시에 여러 숫자 타입을 사용하려면 예상치 못한 동작을 피하기 위해 각 변수를 명시적으로 변환 해야함.

## 원시타입 리터럴 및 숫자 연산

윈시타입  변환이 아닌  리터럴을 직접 사용할 때는 변환이 필요 없다.
...
산술 연산자는 적당한n 타입의 값을 받도록 이미 오버로드 되어있다.
